    ...
    for _, rootpath in pairs(RootPath) do
        ...
        for line in io.lines() do
            ...
            if FileName ~= nil then
                ...
                local sysPath = string.sub(line, #rootpath + 2, #line - 4)
...
function HU.HotUpdateCode(LuaPath, SysPath)
    ...
    log.color("1. HU.HotUpdateCode ", LuaPath, SysPath, OldObject)
    if OldObject then
        HU.VisitedSig = {}
        HU.ChangedFuncList = {}
...
function HU.GetTableSignature(t)
    ...
    if t and t.GetClassType then
        metaTable = t.GetClassType()
        return tostring(metaTable)
    elseif t and t.__tostring and t.__cname then -- todo
...
HU.Init("hotupdatelist", { "F:\\Tools\\lua_hotupdate-master\\reload" }) --please replace the second parameter with you src path
